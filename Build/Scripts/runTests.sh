...
read -r -d '' HELP <<EOF
session_popup test runner. Execute unit, functional and other test suites in
...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="Web/typo3conf/ext/session_popup/${1}"
else
    case ${TEST_SUITE} in
        functional)
            TEST_FILE="Web/typo3conf/ext/session_popup/Tests/Functional"
            ;;
        unit)
            TEST_FILE="Web/typo3conf/ext/session_popup/Tests/Unit"
            ;;
    esac
fi
